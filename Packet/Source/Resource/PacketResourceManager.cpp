...
void PacketResourceManager::Update()
{
	...
	for (int i = m_ReplaceQueue.size() - 1; i >= 0; i--)
	{
		...
		std::unique_ptr<PacketResource> oldResource = m_ResourceStoragePtr->ReplaceObject(objectPtr, hash);
		...
		// TODO: We should use synchronous deletion here?
		m_DeletionQueue.push_back({ std::move(oldResource), true });
...
void PacketResourceManager::OnResourceDataChanged(PacketResource* _resource)
{
	...
	if (std::find(m_ReplaceQueue.begin(), m_ReplaceQueue.end(), _resource) != m_ReplaceQueue.end())
	{
		...
		std::cout << "Found duplicated file modification event on file: \"" << _resource->GetHash().GetPath().String() << "\", ignoring it!" << std::endl;
	...
	auto* resource = m_ResourceStoragePtr->FindObject(_resource->GetHash());
	if (!resource->IsReady())
	{
		// There is no need to set it again, duplicated call
		std::cout << "Found duplicated file modification event on file: \"" << _resource->GetHash().GetPath().String() << "\", ignoring it!" << std::endl;
