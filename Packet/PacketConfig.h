...
static std::string GetRootFolder(std::string _pathString)
{
	...
	auto rootFolderPath = std::experimental::filesystem::path(_pathString);
	while (rootFolderPath.has_parent_path())
	{
		rootFolderPath = rootFolderPath.parent_path();
	}
	return std::experimental::filesystem::path(rootFolderPath).string().append("\\");
}
...
public: //////////
	...
	// Get the file data
	virtual bool GetFileData(uint8_t* _dataOut, uint32_t _bufferSize, Hash _fileHash) = 0;
